...
export default class Dapp {
  ...
  async asyncInitialize() {
    ...
    if (!config.networks[this.networkID]) {
      throw new Error(
        `Could not find network ID ${this.networkID} in configuration.`,
      );
    }

    this.websocketNode = config.networks[this.networkID].websocketNode;
    console.log("Using websocket node", this.websocketNode);
    this.web3Event = new Web3(
      new Web3.providers.WebsocketProvider(this.websocketNode),
    );
  }
  ...
  getContractAddress(name) {
    const configuration = config.networks[this.networkID];
